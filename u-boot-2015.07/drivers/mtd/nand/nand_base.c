 ...
 */
static void nand_decode_ext_id(struct mtd_info *mtd, 
							struct nand_chip *chip,
...
static bool find_full_id_nand(struct mtd_info *mtd, 
							struct nand_chip *chip,
						  						...
						  						struct nand_flash_dev *type)
{
	...
	*dev_id = chip->read_byte(mtd);
	...
	/* type is NULL */
	if (!type) type = nand_flash_ids;
	/*
	 * Find the matching device ID form nand_flash_ids[]
	...
	*/
	...
	/*
	 * If page size == 0,
	 * need to read more configuation from NAND flash
	*/	
	chip->onfi_version = 0;
	/* if pagesize == 0 */
	...
	if (!type->pagesize && chip->init_size) {
		 ...
		 * If pagesize == 0, need to get more infomation 
		 * from NAND
		*/
		nand_decode_ext_id(mtd, chip, id_data, &busw);
	} else {
		/*
		 * The old NAND flash, we can get other information 
		 * for nand table
		*/
...
ident_done:
	 ...
	 * to ensure nand read/write will not overflow
	*/
	...
	chip->bbt_erase_shift = chip->phys_erase_shift =
		ffs(mtd->erasesize) - 1;
	/*
	 * If true, your nand flash size is more than 4GB
	*/
		    ...
		    struct nand_flash_dev *table)
{
	...
	if (IS_ERR(type)) {
		if (!(chip->options & NAND_SCAN_SILENT_NODEV))
			pr_warn("nand: No NAND device found!!!\n");
		chip->select_chip(mtd, -1);
		return PTR_ERR(type);
	}
	...
	if (i > 1) pr_info("nand: %d chips detected\n", i);
