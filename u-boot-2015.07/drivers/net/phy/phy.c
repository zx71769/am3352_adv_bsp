...
static struct phy_device *phy_device_create(struct mii_dev *bus, 
											int addr,
					    					int phy_id,
					    					phy_interface_t interface)
{
												...
												phy_interface_t interface)
{
	if(interface != PHY_INTERFACE_MODE_RMII)
		printf("Phil: error! the phy interface should be RMII!\n");
												...
												phy_interface_t interface)
{
	...
	if (phydev) return phydev;
	/* Try Standard (ie Clause 22) access */
	/* Otherwise we have to try Clause 45 */
	for (i = 0; i < 5; i++) {
		phydev = create_phy_by_mask(bus, 
									phy_mask,
									i ? i : MDIO_DEVAD_NONE, 
									interface);
		if (IS_ERR(phydev)) return NULL;
		if (phydev) return phydev;
	}
 ...
 */
static struct phy_device *get_phy_device(struct mii_dev *bus, 
										int addr,
										phy_interface_t interface)
{
...
int phy_reset(struct phy_device *phydev)
{
	...
	if (reg < 0) {
		debug("phy_reset(721): PHY status read failed\n");
		return -1;
	}
	...
	while ((reg & BMCR_RESET) && timeout--) {
		...
		if (reg < 0) {
			debug("phy_reset(744): PHY status read failed\n");
			return -1;
		}
		udelay(1000);
	}
...
int miiphy_reset(const char *devname, unsigned char addr)
{
	...
	phydev = get_phy_device(bus, addr, PHY_INTERFACE_MODE_RMII);	

	return phy_reset(phydev);
}

struct phy_device *phy_find_by_mask(struct mii_dev *bus, 
									unsigned phy_mask,
									phy_interface_t interface)
{
	/* Reset the bus */
...
#ifdef CONFIG_DM_ETH
struct phy_device *phy_connect(struct mii_dev *bus, 
								int addr,
								struct udevice *dev, 
								phy_interface_t interface)
#else
...
#endif
{
	...
	if (phydev)
		...
		printf("<<ERROR>> Could not get PHY for %s: addr %d\n", bus->name, addr);
