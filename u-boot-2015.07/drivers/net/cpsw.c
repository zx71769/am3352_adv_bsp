...
static int cpdma_process(struct cpsw_priv *priv, 
...
static void cpsw_slave_setup(struct cpsw_slave *slave, 
							int slave_num,
							struct cpsw_priv *priv)
{
	void			*regs = priv->regs;
...
/*
 * Phil add 
 * the gmii_sel control register setting
*/
static void cpsw_gmii_sel_set(struct cpsw_priv *priv)
{
	u32 reg;	
	u32 mask;
	...
	/*
	 * According to ti, suggest pull high those bit
	*/	
	mask = (RGMII1_IDMODE|RGMII2_IDMODE|RMII1_IO_CLK_EN|RMII2_IO_CLK_EN);
	reg = readl(priv->data.gmii_sel);
	phy_if = priv->phydev->interface;
	
	switch(phy_if){
		case PHY_INTERFACE_MODE_MII:
			reg |= (MII_MODE_ENABLE | mask);
			break;
		case PHY_INTERFACE_MODE_RMII:
			reg |= (RMII_MODE_ENABLE | mask);
			break;
		case PHY_INTERFACE_MODE_RGMII:
			reg |= (RGMII_MODE_ENABLE | mask);
			break;
