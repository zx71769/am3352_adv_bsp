...
__weak void __noreturn jump_to_image_no_args(struct spl_image_info *spl_image)
{
	...
	debug("%s: image entry point: 0x%X\n", __func__, spl_image->entry_point);
	image_entry();
}
...
void board_init_r(gd_t *dummy1, ulong dummy2)
{
	...
	case BOOT_DEVICE_ONENAND:
		debug("Boot device ONENAND\n");
		spl_onenand_load_image();
		break;
#endif
#ifdef CONFIG_SPL_NOR_SUPPORT
	case BOOT_DEVICE_NOR:
		debug("Boot device NOR\n");
		spl_nor_load_image();
		break;
#endif
#ifdef CONFIG_SPL_YMODEM_SUPPORT
	case BOOT_DEVICE_UART:
		debug("Boot device UART\n");
		spl_ymodem_load_image();
		break;
#endif
#ifdef CONFIG_SPL_SPI_SUPPORT
	case BOOT_DEVICE_SPI:
		debug("Boot device SPI\n");
		spl_spi_load_image();
		break;
#endif
#ifdef CONFIG_SPL_ETH_SUPPORT
	case BOOT_DEVICE_CPGMAC:
		debug("Boot device CPGMAC\n");
	...
	case BOOT_DEVICE_USBETH:
		debug("Boot device USB_ETH\n");
		spl_net_load_image("usb_ether");
		break;
#endif
#ifdef CONFIG_SPL_USB_SUPPORT
	case BOOT_DEVICE_USB:
		debug("Boot device USB\n");
		spl_usb_load_image();
		break;
#endif
#ifdef CONFIG_SPL_SATA_SUPPORT
	case BOOT_DEVICE_SATA:
		debug("Boot device SATA\n");
		spl_sata_load_image();
		break;
#endif
#ifdef CONFIG_SPL_BOARD_LOAD_IMAGE
	case BOOT_DEVICE_BOARD:
		debug("Boot device BOARD\n");
	...
	switch (spl_image.os) {
	case IH_OS_U_BOOT:
		debug("%s: Jumping to U-Boot\n", __func__);
		break;
#ifdef CONFIG_SPL_OS_BOOT
	case IH_OS_LINUX:
		debug("%s: Jumping to Linux\n", __func__);
