...
static ledmap_t	ixp435_std;
static leddef_t	ixp435_def;
static void ledman_initarch(void);
static void	ixp435_tick(void);
static void ixp435_set(unsigned long bits);
#endif
...
//jinxin add to define W90N740
#ifdef CONFIG_ARCH_W90N740
extern ledmap_t	winbond_std;
extern leddef_t	winbond_def;
static void	winbond_tick(void);
static void ledman_initarch(void);
static void winbond_set(unsigned long bits);
#endif
//jinxin add end
...
ledmode_t led_mode[] = {
	...
	{ "std", ixp435_std, ixp435_def, ledman_bits, ixp435_tick, ixp435_set, HZ/2},
#endif
...
#endif
//jinxin add to define W90n740
#if defined(CONFIG_ARCH_W90N740)
	{ "std", winbond_std, winbond_def, ledman_bits, winbond_tick, winbond_set, HZ / 2},
#endif
//jinxin add end
	{ "",  NULL, NULL, NULL }
};
...
#if defined(CONFIG_ARCH_W90N740)
	unsigned long tmp;
        tmp = (CSR_READ(GPIO_DATAOUT) & ~0xC) | 0xC;
        CSR_WRITE(GPIO_DATAOUT, tmp);
        tmp = (CSR_READ(GPIO_DATAOUT) & ~0xC);
        CSR_WRITE(GPIO_DATAOUT, tmp);
#endif
...
__define_initcall("1", ledman_init,1);
/****************************************************************************/
...
#ifdef CONFIG_ARCH_IXP4XX	
	if(cmd == LEDMAN_CMD_GET)
	{
		gpio_line_get(10, &temp);
		copy_to_user((void *)arg, &temp, sizeof(unsigned long));
		return(0);	
	}
#endif
...
/*
 *	Here is the definition of the LEDs on the CyberGuard/SG590.
 *
 *	LED -  D2   D3   D4   D5   D6   D7
 *	HEX - 0080 0040 0020 0010 0008 0004
 */
static ledmap_t	snapgear425_std = {
	0x0fc, 0x000, 0x080, 0x010, 0x010, 0x010, 0x010, 0x040, 0x040, 0x040,
	0x040, 0x000, 0x000, 0x000, 0x000, 0x030, 0x0cc, 0x004, 0x0fc, 0x000,
	0x000, 0x000, 0x008, 0x000, 0x000, 0x000, 0x000, 0x000, 0x000, 0x020,
	0x020, 0x000, 0x000, 0x000
};
...
#define	ERASEGPIO	10
#define ERASEPIN    IRQ_IXP4XX_GPIO10
#define ERASEIRQ    27
#else
#define	ERASEGPIO	9
#define ERASEPIN    IRQ_IXP4XX_GPIO9
#define ERASEIRQ    26
#endif
...
static void ledman_initarch(void)
{
	...
	*IXP4XX_GPIO_GPOER &= ~LEDMASK;
	...
	set_irq_type(ERASEPIN, IRQT_FALLING);
...
/****************************************************************************/
#endif /* CONFIG_ARCH_SE4000 || CONFIG_MACH_ESS710 || CONFIG_MACH_SG560 || CONFIG_MACH_SG565 || CONFIG_MACH_SG580 || CONFIG_MACH_SG640 || CONFIG_MACH_SG720 || CONFIG_MACH_SG590 */
/****************************************************************************/
/****************************************************************************/
 ...
 */
static ledmap_t	ixp435_std = {
	0x00000030, /* LEDMAN_ALL */
	0x00000000,	/* LEDMAN_POWER */
	0x00000020,	/* LEDMAN_HEARTBEAT */
	0x00000000,	/* LEDMAN_COM1_RX */
	0x00000000,	/* LEDMAN_COM1_TX */
	0x00000000,	/* LEDMAN_COM2_RX */
	0x00000000,	/* LEDMAN_COM2_TX */
	0x00000000,	/* LEDMAN_LAN1_RX */
	0x00000000,	/* LEDMAN_LAN1_TX */
	0x00000000,	/* LEDMAN_LAN2_RX */
	0x00000000,	/* LEDMAN_LAN2_TX */
	0x00000000,	/* LEDMAN_USB1_RX */
	0x00000000,	/* LEDMAN_USB1_TX */
	0x00000000,	/* LEDMAN_USB2_RX */
	0x00000000,	/* LEDMAN_USB2_TX */
	0x00000000,	/* LEDMAN_NVRAM_1 */
	0x00000000,	/* LEDMAN_NVRAM_2 */
	0x00000000,	/* LEDMAN_VPN */
	0x00000000,	/* LEDMAN_LAN1_DHCP */
	0x00000000,	/* LEDMAN_LAN2_DHCP */
	0x00000000,	/* LEDMAN_COM1_DCD */
	0x00000000,	/* LEDMAN_COM2_DCD */
	0x00000000,	/* LEDMAN_ONLINE */
	0x00000000,	/* LEDMAN_LAN1_LINK */
	0x00000000,	/* LEDMAN_LAN2_LINK */
	0x00000000,	/* LEDMAN_VPN_RX */
	0x00000000,	/* LEDMAN_VPN_TX */
	0x00000010,	/* LEDMAN_RESET */
	0x00000000,	/* LEDMAN_STATIC */
	0x00000000,	/* LEDMAN_LAN3_RX */
	0x00000000,	/* LEDMAN_LAN3_TX */
	0x00000000,	/* LEDMAN_LAN3_LINK */
	0x00000000,	/* LEDMAN_LAN3_DHCP */
	0x00000000,	/* LEDMAN_FAILOVER */
};
 ...
 *	set the HW
 */
 	new_value = (~new_value) & 0x00000030;
 	(*lmp->set)(new_value);
	leds_set[0] = leds_set[1] = 0;
}
...
static void ixp435_set(unsigned long bits)
{
	*IXP4XX_GPIO_GPOUTR = (*IXP4XX_GPIO_GPOUTR & ~ixp435_std[0]) | (bits & ixp435_std[0]);
	//if( (bits & 0x00003000)==0x00003000 )
	//	GPIO_DATA_SET_REG = 0x00003000;
	//else
	//	GPIO_DATA_CLEAR_REG = 0x00003000;
}

static void ledman_initarch(void)
{
	gpio_line_config(4, IXP4XX_GPIO_OUT);
	gpio_line_config(5, IXP4XX_GPIO_OUT);
	gpio_line_config(10, IXP4XX_GPIO_IN);
	gpio_line_set(4, 1);
	gpio_line_set(5, 1);
	//GPIO_PIN_DIR_REG |= 0x00003000;
	//GPIO_DATA_SET_REG = 0x00003000;
	//GPIO_DATA_CLEAR_REG = 0x00003000;
}

/****************************************************************************/
#endif /* CONFIG_ARCH_IXP4XX */
/****************************************************************************/
...
static irqreturn_r ledman_interrupt(int irq, void *dev_id, struct pt_regs *regs)
{
...
#ifdef CONFIG_LEDMAN_EKI1524
// to mapping each led to a physical GPIO
static ledmap_t	winbond_std = {
//	0x0000000C, /* LEDMAN_ALL */
	0x00000030, /* LEDMAN_ALL */
	0x00000000,	/* LEDMAN_POWER */
//	0x00000004,	/* LEDMAN_HEARTBEAT */
	0x00000020,	/* LEDMAN_HEARTBEAT */
	0x00000000,	/* LEDMAN_COM1_RX */
	0x00000000,	/* LEDMAN_COM1_TX */
	0x00000000,	/* LEDMAN_COM2_RX */
...
#ifdef CONFIG_LEDMAN_EKI1524
// to define initialize state
static leddef_t	winbond_def = {
	0x00000000,	/*	SET		*/
	0x00000000,	/*	ON		*/
	0x00000000,	/*	OFF		*/
	0x00000030,	/*	FLASH	*/
 ...
 */
	...
	if (!flash_on)
		...
		new_value &= ~((leds_flash[0]&~leds_alt) | (leds_flash[1]&leds_alt));
#ifdef _DEBUG_LEDMAN_TICK_
printk(KERN_NOTICE "leds_flash[0]:%x, leds_set[0]:%x, leds_on[0]:%x, leds_off[0]:%x, leds_alt:%x\n",
	leds_flash[0], leds_set[0], leds_on[0], leds_off[0], leds_alt);
printk(KERN_NOTICE "leds_flash[1]:%x, leds_set[1]:%x, leds_on[1]:%x, leds_off[1]:%x, leds_alt:%x\n",
	(((leds_set[1] | leds_on[1]) & ~leds_off[1]) & ~leds_alt));
printk("new_value:%x\n", new_value);
#endif // #ifdef _DEBUG_LEDMAN_TICK_
/*
 *	set the HW
 */
#ifdef CONFIG_LEDMAN_EKI1524
 	new_value = (~new_value) & 0x30;
#else
 	new_value = (~new_value) & 0xC;
#endif
 	(*lmp->set)(new_value);
	leds_set[0] = leds_set[1] = 0;
}
...
static void winbond_set(unsigned long bits)
{
	unsigned long tmp = (CSR_READ(GPIO_DATAOUT) & ~winbond_std[0]) | bits;
#ifdef _DEBUG_W90N740_SET_
	printk(KERN_NOTICE "winbond_set() bits:0x%X, value:0x%X\n", bits, tmp);
#endif // #ifdef _DEBUG_WINBOND_SET_
	CSR_WRITE(GPIO_DATAOUT, tmp);
}

static void ledman_initarch(void)
{
//	printk(KERN_NOTICE "ledman_initarch()\n");
#ifdef CONFIG_LEDMAN_EKI1524
	printk("CONFIG_LEDMAN_EKI1524\n");
	CSR_WRITE(GPIO_CFG, CSR_READ(GPIO_CFG) & 0xFFFFFFF3);
	CSR_WRITE(GPIO_DIR, (CSR_READ(GPIO_DIR) & 0xFFFFFFCF) | 0x30);
	CSR_WRITE(GPIO_DATAOUT, (CSR_READ(GPIO_DATAOUT) & 0xFFFFFFCF) | 0x30);
#else
	printk("EKI1122L LEDMAN\n");
	CSR_WRITE(GPIO_CFG, CSR_READ(GPIO_CFG) & 0xFFFFFFFC);
	CSR_WRITE(GPIO_DIR, (CSR_READ(GPIO_DIR) & 0xFFFFFFF0) | 0xF);
	CSR_WRITE(GPIO_DATAOUT, (CSR_READ(GPIO_DATAOUT) & 0xFFFFFFF3) | 0xC);
#endif
//	CSR_WRITE(GPIO_CFG, CSR_READ(GPIO_CFG) & 0xFFFFFFFC);
//	CSR_WRITE(GPIO_DIR, (CSR_READ(GPIO_DIR) & 0xFFFFFFF0) | 0xF);
//	CSR_WRITE(GPIO_DATAOUT, (CSR_READ(GPIO_DATAOUT) & ~winbond_std[0]) | winbond_std[0]);
}

/****************************************************************************/
#endif /* CONFIG_ARCH_WINBOND */
/****************************************************************************/
